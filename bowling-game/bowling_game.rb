...
class Game
  ...
  def roll(points)
    ...
    raise 'Points score invalid, 0-10 allowed' if points > 10 || points.negative?
  ...
  def bonifications
    @game[@turn - 1] << @game[@turn][0] if @game[@turn - 1].compact.sum == 10 && @game[@turn - 1][0] != 10
    @game[@turn - 2] << (@game[@turn - 1][0] + @game[@turn][0]) if @game[@turn - 2][0] == 10
  end
end
