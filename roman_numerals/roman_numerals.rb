...
# https://programmerclick.com/article/9912978100/
...
module RomanNumerals
  ...
  def self.to_roman(number)
    ...
    (splited_number.size - 1).downto(0) do |i|
      ...
      result << if EQUIVALENCES.values.include?(splited_number[i] * (10**i))
                  EQUIVALENCES[splited_number[i] * (10**i)]
                else
                  compute_roman_digit(splited_number, i)
                end
    end
